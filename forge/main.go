...
import (
	...
	"github.com/frantjc/daggerverse/forge/internal/dagger"
...
const (
	// actionsCacheImageReference is the container image reference
	// that is used as the actions cache service for GitHub Actions.
	actionsCacheImageReference = "ghcr.io/falcondev-oss/github-actions-cache-server:8"
...
func (m *Forge) Use(
	...
	case githubactions.RunsUsingNode12:
		container = container.From(forge.Node12ImageReference)
	case githubactions.RunsUsingNode16:
		container = container.From(forge.Node16ImageReference)
	case githubactions.RunsUsingNode20:
		container = container.From(forge.Node20ImageReference)
	case githubactions.RunsUsingNode24:
		container = container.From(forge.Node24ImageReference)
	default:
...
func withActionsCache(container *dagger.Container) *dagger.Container {
	var (
		alias = "actions-cache"
		port = 3000
		actionsResultsURL     = fmt.Sprintf("http://%s:%d", alias, port)
	...
	return container.
		...
		WithEnvVariable(/* TODO(frantjc): githubactions.EnvVarActionsRuntimeToken */ "ACTIONS_RUNTIME_TOKEN", "fake")
}
